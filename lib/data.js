...
//
// Set WHATSAPP_NUMBER to the number in international format, digits only,
// no "+" and no spaces (e.g. "919876543210"). It is deliberately null
// until Harshit confirms which number should be published — publishing a
// guessed number would be worse than not offering the channel — and every
// consumer checks WHATSAPP_URL for null and falls back to the booking
// link, so nothing breaks while it is unset.
...
// number can be written however is readable (+91 93357 85136) without
...
// A tap-to-call link built from the same digits already published as the
// WhatsApp number, so this reveals nothing new. It exists because a local
// services buyer who is ready to talk will call — and until now the site
// gave them nowhere to do that. `PHONE_DISPLAY` is the human-readable form
// for on-screen text; `PHONE_URL` is the tel: href.
export const PHONE_DISPLAY = WHATSAPP_DIGITS
  ? `+${WHATSAPP_DIGITS.slice(0, 2)} ${WHATSAPP_DIGITS.slice(2, 7)} ${WHATSAPP_DIGITS.slice(7)}`
  : null;

export const PHONE_URL = WHATSAPP_DIGITS ? `tel:+${WHATSAPP_DIGITS}` : null;
